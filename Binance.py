...
binance_keys = {
    'api_key': "wh8Ngdr0401sNA3xrw8ei9yc04KEe2Sbi83KtqkbbdWdANhHrxUTmSMBy2g3ws2H ",
    'secret_key': "wPWn90fnpnVjD3WBo3AiwcS0Wzq7cd8XP1Abke6KMcJBy8zOCYtOan2PpHffdhlF"
}
...
class Binance:
    def __init__(self):
        ...
        '''#my client
        self.client = Client(binance_keys['api_key'], binance_keys['secret_key'], {"verify": False, "timeout": 20})
        self.bm = BinanceSocketManager(client)
        self.df = []
        # dcm = DepthCacheManager(client, 'BNBBTC', callback=process_depth, bm=bm, refresh_interval=60*60)
        self.conn_key = bm.start_kline_socket('BNBBTC', process_message, interval=KLINE_INTERVAL_30MINUTE)
...
'''
    ...
    def GetTradingSymbols(self, quoteAssets: list = None):
        ...
        symbole_list = ['THETAETH', 'LINKETH', 'LTCETH', 'BNBETH', 'BATETH', 'TRXETH', 'STEEMETH', 'EOSETH', 'ADAETH']
        return symbole_list
        ...
        for pair in data['symbols']:
            if pair['symbol'] == 'LTCETH' or pair['symbol'] == 'BNBETH' or pair['symbol'] == 'BATETH' \
                    or pair['symbol'] == 'THETAETH' or pair['symbol'] == 'LINKETH'\
                    or pair['symbol'] == 'TRXETH' or pair['symbol'] == 'STEEMETH'\
                    or pair['symbol'] == 'EOSETH' or pair['symbol'] == 'ADAETH':
